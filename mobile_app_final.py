...
API_BASE_URL = os.getenv('CDC_API_BASE_URL', DEFAULT_API_BASE_URL).rstrip('/')
REQUEST_TIMEOUT = float(os.getenv('CDC_REQUEST_TIMEOUT', '8'))
...
class CDCApp:
    ...
    def _safe_json(response):
        """Parse JSON safely without raising."""
        try:
    ...
    @staticmethod
    def _extract_error(response, data):
        """Extract error message from JSON or text response."""
    ...
    def _api_get(self, path, params=None):
        """GET wrapper: returns (ok, data, err_msg)."""
        try:
            resp = requests.get(self._build_url(path), params=params, timeout=REQUEST_TIMEOUT)
        except requests.RequestException as e:
            return False, {}, str(e)
        data = self._safe_json(resp)
        if 200 <= resp.status_code < 300:
            return True, data, ''
        return False, data, self._extract_error(resp, data)

    def _api_post(self, path, payload):
        """POST wrapper (JSON): returns (ok, data, err_msg)."""
        try:
            resp = requests.post(self._build_url(path), json=payload, timeout=REQUEST_TIMEOUT)
        except requests.RequestException as e:
            return False, {}, str(e)
        data = self._safe_json(resp)
        if 200 <= resp.status_code < 300:
    ...
    def handle_login(self, user_id, role):
        ...
        if role == "household":
            ...
            ok, data, err = self._api_get(f"/households/{user_id}")
            if ok:
                self.household_data = data.get("household", {})
                self.show_household_dashboard()
            else:
                self.show_snackbar(err or "Household not found", ft.colors.RED)
    ...
    def handle_household_signup(self, name, nric, email, postal, unit):
        ...
        if ok:
            ...
            self.show_snackbar(err or "Registration failed", ft.colors.RED)
# Display merchant registration page
    def show_merchant_signup(self):
        ...
        def on_bank_change(e):
            ...
            branches = []
            ok, data, _ = self._api_get(
                f"/banks/{bank_code}/branches",
    ...
    def claim_voucher_tranche(self, tranche_id):
        ...
        ok, data, err = self._api_post(
            "/vouchers/claim",
            {"household_id": self.current_user, "tranche_id": tranche_id},
        )

        if ok:
            self.show_snackbar(
                data.get("message", "Vouchers claimed successfully!"),
                ft.colors.GREEN,
            )

            # Refresh household data
            ok2, data2, _ = self._api_get(f"/households/{self.current_user}")
            if ok2:
                self.household_data = data2.get("household", {})

            # Return to voucher redemption page
            self.show_voucher_claim()
        else:
            self.show_snackbar(err or "Claim failed", ft.colors.RED)
    ...
    def show_transactions(self):
        ...
        transactions = []
        ok, data, _ = self._api_get("/transactions")
        if ok:
            ...
            try:
                ...
                def confirm_redemption(e):
                    ...
                    if ok:
                        ...
                        self.show_snackbar(err or "Redemption failed", ft.colors.RED)
    ...
    def show_snackbar(self, message, color=ft.colors.BLUE):
        ...
        try:
            ...
            try:
                self.page.overlay[:] = [
                    c for c in self.page.overlay if c.__class__.__name__ != "SnackBar"
                ]
            except Exception:
                pass
